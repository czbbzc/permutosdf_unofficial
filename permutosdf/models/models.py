...
from permuto.volume_rendering.volume_rendering_modules import *
from permuto.models.modules import CreateRaysModule
from permuto.utils.common_utils import map_range_val
from permuto.utils.common_utils import leaky_relu_init
from permuto.utils.common_utils import apply_weight_init_fn
# from permuto_sdf import PermutoSDF
# from permuto_sdf import RaySamplesPacked
# from permuto.utils.common_utils import TIME_START
# from permuto.utils.common_utils import TIME_END

import permutohedral_encoding as permuto_enc
from py_permuto_sdf import RaySamplesPacked
...
class SDF(torch.nn.Module):
    ...
    def __init__(self, in_channels, boundary_primitive, geom_feat_size_out, nr_iters_for_c2f):
        ...
        self.boundary_primitive=boundary_primitive # aabb
        self.geom_feat_size_out=geom_feat_size_out # 32
        ...
        with torch.set_grad_enabled(False):
            self.mlp_sdf[-1].bias+=self.sdf_shift # type: ignore #faster if we just put it in the bias
...
class RGB(torch.nn.Module):
    ...
    def __init__(self, in_channels, boundary_primitive, geom_feat_size_in, nr_iters_for_c2f, implementation: Literal["tcnn", "torch"] = "tcnn",):
    ...
    def forward(self, points, samples_dirs, sdf_gradients, geom_feat,  iter_nr, model_colorcal=None, img_indices=None, ray_start_end_idx=None):
        ...
        with torch.set_grad_enabled(False):
            ...
            samples_dirs_enc=self.direction_encoding(samples_dirs)
        #normals
...
class NerfHash(torch.nn.Module):
    ...
    def __init__(self, in_channels, boundary_primitive, nr_iters_for_c2f, implementation: Literal["tcnn", "torch"] = "tcnn",):
...
#         self.textures = torch.nn.ParameterList()
